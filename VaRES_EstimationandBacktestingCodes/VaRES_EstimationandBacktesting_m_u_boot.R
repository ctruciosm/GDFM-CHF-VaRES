...
s_GDFM_Boot = read.table("s_GDFM-CHF_Boot.txt")



sigma = cbind(as.numeric(s_RM2006[1,]),as.numeric(s_DCC[1,]),as.numeric(s_ABC[1,]),as.numeric(s_GDFM[1,]),as.numeric(s_GDFM_Boot[1,]))
...
for(l in 1:n){
  ...
  VaR1m[l,] = c(quantile(boot_RM2006m, prob = a1),quantile(boot_DCCm, prob = a1),quantile(boot_ABCm, prob = a1), quantile(boot_GDFMm, prob = a1), quantile(boot_GDFM_Boot_m, prob = a1))
  VaR2m[l,] = c(quantile(boot_RM2006m, prob = a2),quantile(boot_DCCm, prob = a2),quantile(boot_ABCm, prob = a2), quantile(boot_GDFMm, prob = a2), quantile(boot_GDFM_Boot_m, prob = a2))
  VaR5m[l,] = c(quantile(boot_RM2006m, prob = a5),quantile(boot_DCCm, prob = a5),quantile(boot_ABCm, prob = a5), quantile(boot_GDFMm, prob = a5), quantile(boot_GDFM_Boot_m, prob = a5))
  
  ES1m[l,] = c(mean(boot_RM2006m[boot_RM2006m < VaR1m[l,1]]), mean(boot_DCCm[boot_DCCm < VaR1m[l,2]]),
               mean(boot_ABCm[boot_ABCm < VaR1m[l,3]]), mean(boot_GDFMm[boot_GDFMm < VaR1m[l,4]]),
               mean(boot_GDFM_Boot_m[boot_GDFM_Boot_m < VaR1m[l,5]]))
  ES2m[l,] = c(mean(boot_RM2006m[boot_RM2006m < VaR2m[l,1]]), mean(boot_DCCm[boot_DCCm < VaR2m[l,2]]),
               mean(boot_ABCm[boot_ABCm < VaR2m[l,3]]), mean(boot_GDFMm[boot_GDFMm < VaR2m[l,4]]),
               mean(boot_GDFM_Boot_m[boot_GDFM_Boot_m < VaR2m[l,5]]))
  ES5m[l,] = c(mean(boot_RM2006m[boot_RM2006m < VaR5m[l,1]]), mean(boot_DCCm[boot_DCCm < VaR5m[l,2]]),
               mean(boot_ABCm[boot_ABCm < VaR5m[l,3]]), mean(boot_GDFMm[boot_GDFMm < VaR5m[l,4]]),
